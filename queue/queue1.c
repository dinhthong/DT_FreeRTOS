...
{  
    ...
    for(;;) 
    {
	xQueueSend(xQueue, (void*)&data_s, (portTickType)0); //doi so thu 3, timeout: so 0 co nghia neu message day thi bo qua luon
	data_s++;
	xQueueSend(xQueue, (void*)&data_s, (portTickType)0);//xQueueSend(xQueue, (void*)&data_s, (portTickType)0xFFFFFFFFF);
	data_s++;
	xQueueSend(xQueue, (void*)&data_s, (portTickType)0);
	data_s++;
	xQueueSend(xQueue, (void*)&data_s, (portTickType)0);
	data_s++;
	xQueueSend(xQueue, (void*)&data_s, (portTickType)0);
	data_s++;
	vTaskDelay(1000); // thay 1000 -> 1 > hien tuong nghen co chai > can thiet ke timeout.} vi du dat maximum time out 0xFFFFFFFFF.... 
...
{ 
    ...
    for(;;) 
    {
	if(xQueueReceive(xQueue, (void*)&data_g, (portTickType)0xFFFFFFFF))
	{
	    printf("data_g:  %u\r\n", data_g);
	}
    }
